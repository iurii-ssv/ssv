...
func TestDiscV5Service_checkPeer(t *testing.T) {
	...
	// Valid peer
	err := dvs.checkPeer(ToPeerEvent(NewTestingNode(t)))
	require.NoError(t, err)

	// No domain
	err = dvs.checkPeer(ToPeerEvent(NodeWithoutDomain(t)))
	require.ErrorContains(t, err, "could not read domain type: not found")

	// No next domain. No error since it's not enforced
	err = dvs.checkPeer(ToPeerEvent(NodeWithoutNextDomain(t)))
	require.NoError(t, err)

	// Matching main domain
	err = dvs.checkPeer(ToPeerEvent(NodeWithCustomDomains(t, testNetConfig.DomainType(), spectypes.DomainType{})))
	require.NoError(t, err)

	// Matching next domain
	err = dvs.checkPeer(ToPeerEvent(NodeWithCustomDomains(t, spectypes.DomainType{}, testNetConfig.DomainType())))
	require.NoError(t, err)

	// Mismatching domains
	err = dvs.checkPeer(ToPeerEvent(NodeWithCustomDomains(t, spectypes.DomainType{}, spectypes.DomainType{})))
	require.ErrorContains(t, err, "mismatched domain type: neither 00000000 nor 00000000 match 00000302")

	// No subnets
	err = dvs.checkPeer(ToPeerEvent(NodeWithoutSubnets(t)))
	require.ErrorContains(t, err, "could not read subnets: not found")

	// Zero subnets
	err = dvs.checkPeer(ToPeerEvent(NodeWithZeroSubnets(t)))
	...
	subnets[10] = 1
	err = dvs.checkPeer(ToPeerEvent(NodeWithCustomSubnets(t, subnets)))
